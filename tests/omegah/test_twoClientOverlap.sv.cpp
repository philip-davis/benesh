...
redev::ClassPtn setupClientPartition(Omega_h::Mesh& mesh, int clientId) {
  std::stringstream ss;
  ss << "appPartition" << clientId;
  ts::writeVtk(mesh,ss.str(),0);
...
void client(Omega_h::Mesh& mesh, std::string fieldName, const int clientId) {
  ...
  auto partition = setupClientPartition(mesh, clientId);
  ...
  std::cout << "rank = " << rank << " msgs.size() = " << msgs.size() << std::endl;
  int j=0;
  for(size_t i=0; i<gids_h.size(); i++) {
    if( isOverlap_h[i] ) {
