                     ...
                     const ClassificationPartition &partition)
{
    ...
    if(!rank) {
        ...
        for(auto iter = elemsPerRank.begin(); iter != elemsPerRank.end();
            iter++) {
            const auto dest = iter->first;
            REDEV_ALWAYS_ASSERT(
                dest < ohComm->size()); // the destination rank must exist
            if(dest) {
                       ...
                       const redev::GOs &vtxData, const redev::GOs &rdvPermute)
{
    ...
    auto ohStr = std::string(name); // omegah wants a std::string const
                                    // reference
